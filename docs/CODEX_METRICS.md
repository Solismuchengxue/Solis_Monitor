...
小屏“周使用 TOKEN”使用主周额度的重置时间确定七天周期，并从所有主任务的 `token_count` 事件增量汇总本周期 Token；子代理任务不计入。账户累计读取成功时，桌面端还会用当前 `lifetimeTokens` 与该周期持久化基线做差。

账户周期差值大于零时以账户值为准；账户接口仍返回滞后的零差值、但本地周期汇总已经大于零时，保留本地汇总，避免有效用量被显示为 `0.00 亿`。两种来源都为零时仍显示零。该数值不根据周额度已用百分比反推，因为额度事件没有提供可用于换算的 Token 总上限。
